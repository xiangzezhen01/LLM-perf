...
class Chinchilla(ScalingMethod):
    ...
    def fit(self, df, method='all'):
        ...
        params = Parameters()
        if method == 'D':
            params.add('A', value=406.4, min=400, max=410, vary=False)
            params.add('B', value=410.7, min=405, max=415, vary=True)
            params.add('E', value=1.69, min=1.2, max=1.8, vary=True)
            params.add('alpha', value=0.34, min=0.2, max=0.5, vary=False)
            params.add('beta', value=0.28, min=0.1, max=0.5, vary=True)
        elif method == 'N':
            params.add('A', value=406.4, min=400, max=410, vary=True)
            params.add('B', value=410.7, min=405, max=415, vary=False)
            params.add('E', value=1.69, min=1.2, max=1.8, vary=True)
            params.add('alpha', value=0.34, min=0.2, max=0.5, vary=True)
            params.add('beta', value=0.28, min=0.1, max=0.5, vary=False)
        else:
            params.add('A', value=406.4, min=400, max=410, vary=True)
            params.add('B', value=410.7, min=405, max=415, vary=True)
            params.add('E', value=1.69, min=1.2, max=1.8, vary=True)
...
class Farseer(ScalingMethod):
    ...
    def fit(self, df, method='all'):
        ...
        params = Parameters()
        if method == 'D':
            params.add('a1', value=-0.124, min=-0.2, max=0, vary=True)
            params.add('alpha', value=0.123, min=0, max=0.2, vary=True)
            params.add('b1', value=0.424, min=0.3, max=0.5, vary=True)
            params.add('a2', value=88.01, min=85, max=90, vary=False)
            params.add('beta', value=-0.1, min=-0.2, max=0, vary=False)
            params.add('b2', value=-6.287, min=-8, max=-5, vary=False)
            params.add('a3', value=-0.021, min=-0.1, max=0, vary=False)
            params.add('gamma', value=0.169, min=0, max=0.3, vary=False)
            params.add('b3', value=-0.091, min=-0.2, max=0, vary=False)
        else:
            params.add('a1', value=-0.124, min=-0.2, max=0, vary=True)
            params.add('alpha', value=0.123, min=0, max=0.2, vary=True)
            params.add('b1', value=0.424, min=0.3, max=0.5, vary=True)
            params.add('a2', value=88.01, min=85, max=90, vary=True)
            params.add('beta', value=-0.1, min=-0.2, max=0, vary=True)
            params.add('b2', value=-6.287, min=-8, max=-5, vary=True)
            params.add('a3', value=-0.021, min=-0.1, max=0, vary=True)
            params.add('gamma', value=0.169, min=0, max=0.3, vary=True)
            params.add('b3', value=-0.091, min=-0.2, max=0, vary=True)
...
class Kaplan(ScalingMethod):
    ...
    def fit(self, df, method='all'):
        ...
        params = Parameters()
        if method == 'N':
            params.add('Nc', value=6.4, min=0.01, max=10, vary=True)
            params.add('Dc', value=1.8, min=0.01, max=10, vary=False)
            params.add('aN', value=0.076, min=0.01, max=0.1, vary=True)
            params.add('aD', value=0.103, min=0.01, max=0.2, vary=False)
        elif method == 'D':
            params.add('Nc', value=6.4, min=0.01, max=10, vary=False)
            params.add('Dc', value=1.8, min=0.01, max=10, vary=True)
            params.add('aN', value=0.076, min=0.01, max=0.1, vary=False)
            params.add('aD', value=0.103, min=0.01, max=0.2, vary=True)
        else:
            params.add('Nc', value=6.4, min=0.01, max=10, vary=True)
            params.add('Dc', value=1.8, min=0.01, max=10, vary=True)
            params.add('aN', value=0.076, min=0.01, max=0.1, vary=True)
...
def scaling_data_constrain(df):
    """
    对DataFrame中列N和D值相同的数据组，保留loss值最小的5条记录

    参数:
        df: pandas DataFrame，包含至少N、D和loss列

    返回:
        pandas DataFrame，处理后的结果
    """
    # 按照N和D列分组，然后在每个组内按loss升序排序，取前5条
    result = df.groupby(['N', 'D']).apply(
        lambda group: group.sort_values('loss').head(5)
...
class combine_scaling(ScalingMethod):
    def __init__(self, scaling_laws, weight_method='softmax', data_constrain=True, T=1):
