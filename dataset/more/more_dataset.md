...
## Scaling Laws with Vocabulary: Larger Models Deserve Larger Vocabularies: https://github.com/sail-sg/scaling-with-vocab

| 字段名称（Field Name）       | 核心定义                                                                 |
|-----------------------------|--------------------------------------------------------------------------|
| `vocab_size`                | 词汇表大小，示例值包括4096、8192、96256等，对应不同实验设置                |
| `embed_dim`                 | 模型的嵌入维度，与非词汇表参数相关                                        |
| `num_characters`            | 训练数据的字符总量（记为H），用于计算FLOPs和损失函数                      |
| `Non_vocab_parameters`      | 非词汇表参数数量（记为Nnv），即模型主体参数，不含词嵌入矩阵                |
| `FLOPs`                     | 训练过程中的计算量（单位：浮点运算次数），用于等FLOPs分析                  |
| `Lossu`                     | 单字归一化损失（Unigram-Normalized 损失），论文提出的词汇表无关损失度量，用于公平比较不同词汇表模型的性能 |

len = 1021
arch num = 7
**note that**: `num_characters`, `Lossu` are not the 'N', 'loss' in other scaling law

## RegMix: Data Mixture as Regression for Language Model Pre-training: https://github.com/sail-sg/regmix
...
**note that**: model size -> training data size
- 1m->1b
- 60m->1b
- 1b->25b
...
## Revisiting Neural Scaling Laws in Language and Vision: https://github.com/google-research/google-research/tree/master/revisiting_neural_scaling_laws

metrixs is normalized
...
| 数据集类型                        | moe预训练           |
| 包含LLM架构数量                    | 7    |
| LLM MAX parameters                   | 15M    |
| LLM MIN parameters                   | 1.3B    |
...
## Selecting Large Language Model to Fine-tune via Rectified Scaling Law: https://github.com/linhaowei1/Fine-tuning-Scaling-Law


## thoughts:
- modeling basic Loss by Scaling law
- modeling other features' contributions and interaction contribution
    - HINNPerf; how to use HiNNPerf?
    - other future setting may depend on N, D (steplaw: Step Law -- Optimal Hyperparameter Scaling Law in Large Language Model Pretraining)