...
"""
'dense1_loss': 'D:\\CodePycharm\\LLMs-Performance-Learning\\dataset\\process\\dense1_loss_arch',

"""
system_dir_dict = {

    'dense3_loss': 'D:\\CodePycharm\\LLMs-Performance-Learning\\dataset\\process\\data_constain_dense',
    'dense2_loss': 'D:\\CodePycharm\\LLMs-Performance-Learning\\dataset\\process\\dense2_loss_arch',
    'moe_loss': 'D:\\CodePycharm\\LLMs-Performance-Learning\\dataset\\process\\moe_loss_arch',
    'moe2_loss': 'D:\\CodePycharm\\LLMs-Performance-Learning\\dataset\\process\\moe2_routing',
    'dense4_loss': 'D:\\CodePycharm\\LLMs-Performance-Learning\\dataset\\process\\data_mixlaw_dense23',
    'dense5_loss': 'D:\\CodePycharm\\LLMs-Performance-Learning\\dataset\\process\\data_mixlaw_dense5'
}
# 
...
system_test_arch_dict = {
    'dense4_loss': [1, 2, 4],
    'dense5_loss': [1, 2, 4],
    'dense1_loss': [1, 4, 8, 12, 16, 24],
    'dense3_loss': [1, 3, 6, 9, 12],
    'dense2_loss': [1, 2, 3, 4],
    'moe_loss': [1, 2, 3],
    'moe2_loss': [1, 2, 3, 5]
}
...
def test_methods(dir_path, n_arch):
    result_dict = {'farseer': [], "chinchilla": [], "kaplan": [], "rf_ND": [], "rf_all": [],
                   'combine_scaling_softmax': [], 'combine_scaling_softmax_constrain': [], 'combine_scaling_softmax_T_0.1':[], 'combine_scaling_softmax_T_0.5':[], 'combine_scaling_softmax_T_1.5':[]}
    for llm_file_name in os.listdir(dir_path):
        if llm_file_name == 'dense1_loss_arch_h_1920_ffnh_5184_numh_30_numl_30.csv':
            print("ok")
        ...
        default_farseer_result = farseer_model.predict(test_df, use_default_params=False)
        ...
        default_chinchilla_result = chinchilla_model.predict(test_df, use_default_params=False)
        ...
        default_kaplan_result = kaplan_model.predict(test_df, use_default_params=False)
        ...
        cs_model = combine_scaling(['farseer', 'chinchilla', 'kaplan', 'rf_nd'], data_constrain=False)
        ...
        cs1_model = combine_scaling(['farseer', 'chinchilla', 'kaplan', 'rf_nd'], data_constrain=True)
        ...
        logging.info(f"==== combine scaling law with softmax T = 0.1 ====")
        cs2_model = combine_scaling(['farseer', 'chinchilla', 'kaplan', 'rf_nd'], data_constrain=False, T=0.1)
        ...
        logging.info(f"==== combine scaling law with softmax T = 0.5 ====")
        cs3_model = combine_scaling(['farseer', 'chinchilla', 'kaplan', 'rf_nd'], data_constrain=False, T=0.5)
        cs3_model.fit(train_df)
        cs3_result = cs3_model.predict(test_df)
        cs3_mape = get_mape(test_y, cs3_result)
        cs3_ape_list = get_ape_list(test_y, cs3_result)

        logging.info(f"==== combine scaling law with softmax T = 1.5 ====")
        cs4_model = combine_scaling(['farseer', 'chinchilla', 'kaplan', 'rf_nd'], data_constrain=False, T=1.5)
        cs4_model.fit(train_df)
        cs4_result = cs4_model.predict(test_df)
        cs4_mape = get_mape(test_y, cs4_result)
        cs4_ape_list = get_ape_list(test_y, cs4_result)
        ...
        result_dict['combine_scaling_softmax_T_0.1'].append(cs2_mape)
        result_dict['combine_scaling_softmax_T_0.5'].append(cs3_mape)
        result_dict['combine_scaling_softmax_T_1.5'].append(cs4_mape)

        print(result_dict)

    return [result_dict['farseer'], result_dict['chinchilla'], result_dict['kaplan'], result_dict['rf_ND'],
            result_dict['rf_all'], result_dict['combine_scaling_softmax'],
            result_dict['combine_scaling_softmax_constrain'], result_dict['combine_scaling_softmax_T_0.1'], result_dict['combine_scaling_softmax_T_0.5'], result_dict['combine_scaling_softmax_T_1.5']]
    ...
    for i in range(repeat_time):
        ...
        for name, dir_path in system_dir_dict.items():
            ...
            for n_arch in test_n_arch:
                ...
                result_dict['combine_scaling_softmax_T_0.1'] = result_list[7]
                result_dict['combine_scaling_softmax_T_0.5'] = result_list[8]
                result_dict['combine_scaling_softmax_T_1.5'] = result_list[9]
                print(result_dict)
                df = pd.DataFrame(result_dict)
                df.to_csv(f'D:\\CodePycharm\\LLMs-Performance-Learning\\result\\test_constrain_rebuild\\{name}_arch_{n_arch}_time{i}.csv', index=False)
